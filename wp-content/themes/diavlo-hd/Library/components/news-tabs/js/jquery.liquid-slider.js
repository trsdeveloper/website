...
(function(jQuery, window, document, undefined) {
  ...
  var Slider = {
    ...
    makeResponsive: function() {
      ...
      jQuery(self.sliderId + '-wrapper').addClass('ls-responsive').css({
        'max-width': jQuery(self.sliderId + ' .panel:first-child').width(),
        'width': '100%'
      });
      // Update widths
      jQuery(self.sliderId + ' .panel-container').css('width', 100 * self.panelCountTotal + self.pSign);
      jQuery(self.sliderId + ' .panel').css('width', 100 / self.panelCountTotal + self.pSign);

      // Cache the padding for add/removing arrows
      if (self.options.hideArrowsWhenMobile) {
        self.leftWrapperPadding = jQuery(self.sliderId + '-wrapper').css('padding-left');
        self.rightWrapperPadding = (self.jQuerysliderWrap).css('padding-right');
      }
    ...
    responsiveEvents: function() {
      ...
      if ((self.jQuerysliderId).outerWidth() < mobileNavChangeOver) {
        if (self.options.mobileNavigation) {
          (self.navigation).css('display', 'none');
          (self.dropdown).css('display', 'block');
          (self.dropdownSelect).css('display', 'block');
          // Update the navigation
          jQuery(self.sliderId + '-nav-select').val(self.options.mobileNavDefaultText);
        }
      ...
      jQuery(self.sliderId + '-wrapper').css('width', '100%');

      // Update when the select box changes
      if (self.options.mobileNavigation) {
        (self.dropdownSelect).change(function() {
          self.setNextPanel(parseInt(jQuery(this).val().split('tab')[1], 10) - 1);
        });
      }
    },
    ...
    addNavigation: function(navClass) {
      var self = this,
        dynamicTabsElm = '<' + self.options.navElementTag + ' class="ls-nav"><ul id="' +
          (self.jQueryelem).attr('id') + '-nav-ul"></ul></' + self.options.navElementTag + '>';
      // Add basic frame
      if (self.options.dynamicTabsPosition === 'bottom') {
        (self.jQuerysliderId).after(dynamicTabsElm);
      } else {
        (self.jQuerysliderId).before(dynamicTabsElm);
      }
      ...
      if (self.options.mobileNavigation) {
        var selectBoxDefault = (self.options.mobileNavDefaultText) ?
          ...
          dropDownList = '<div class="ls-select-box"><select id="' +
            (self.jQueryelem).attr('id') + '-nav-select" name="navigation">' +
            selectBoxDefault + '</select></div>';
        // cache elements
        self.navigation = jQuery(self.sliderId + '-nav-ul').before(dropDownList);
        self.dropdown = jQuery(self.sliderId + '-wrapper .ls-select-box');
        self.dropdownSelect = jQuery(self.sliderId + '-nav-select');

        jQuery.each(
          (self.jQueryelem).find(self.options.panelTitleSelector),
          function(n) {
            jQuery((self.jQuerysliderWrap)).find('.ls-select-box select')
      ...
      jQuery.each(
        (self.jQueryelem).find(self.options.panelTitleSelector),
        function(n) {
          jQuery((self.jQuerysliderWrap)).find('.ls-nav ul').append('<li class="tab' +
            (n + 1) + '"><a class="' + ( navClass || '') + '" href="#' +
            (n + 1) + '">' + self.getNavInsides(this) + '</a></li>');
          if (!self.options.includeTitle) jQuery(this).remove();
        }
      );
    },
    ...
    alignNavigation: function() {
      ...
      if (self.options.dynamicTabsAlign !== 'center') {
        if (!self.options.responsive) {
          jQuery((self.jQuerysliderWrap)).find('.ls-nav ul').css(
            'margin-' + self.options.dynamicTabsAlign,
            // Finds the width of the arrows and the margin
            jQuery((self.jQuerysliderWrap)).find(
              '.ls-nav-' +
              self.options.dynamicTabsAlign +
              arrow
            ).outerWidth(true) + parseInt((self.jQuerysliderId)
              .css('margin-' + self.options.dynamicTabsAlign), 10)
          );
        }
        jQuery((self.jQuerysliderWrap)).find('.ls-nav ul').css('float', self.options.dynamicTabsAlign);
      }
      self.totalNavWidth = jQuery((self.jQuerysliderWrap)).find('.ls-nav ul').outerWidth(true);
      if (self.options.dynamicTabsAlign === 'center') {
        // Get total width of the navigation tabs and center it
        self.totalNavWidth = 0;
        jQuery((self.jQuerysliderWrap)).find('.ls-nav li a').each(function() {
          self.totalNavWidth += jQuery(this).outerWidth(true);
        });
        jQuery((self.jQuerysliderWrap)).find('.ls-nav ul').css('width', self.totalNavWidth + 1);
      }
    },

    registerNav: function() {
      var self = this;
      (self.jQuerysliderWrap).find('[class^=ls-nav] li').on('click', function() {
        self.setNextPanel(parseInt(jQuery(this).attr('class').split('tab')[1], 10) - 1);
        return false;
      });
    },
    ...
    addArrows: function(arrowClass) {
      var self = this,
        arrow = (self.options.dynamicArrowsGraphical) ? "-arrow " : ' ';
      (self.jQuerysliderWrap).addClass("arrows");
      ...
      // Build the arrows
      (self.jQuerysliderId).before('<div class="ls-nav-left' + arrow + (arrowClass || '') +
        '"><a href="#">' +
        self.options.dynamicArrowLeftText + '</a></div>');
      (self.jQuerysliderId).after('<div class="ls-nav-right' + arrow + (arrowClass || '') +
        '"><a href="#">' +
        self.options.dynamicArrowRightText + '</a></div>');

      self.leftArrow = jQuery(self.sliderId + '-wrapper [class^=ls-nav-left]')
        .css('visibility', "hidden").addClass('ls-hidden');
      self.rightArrow = jQuery(self.sliderId + '-wrapper [class^=ls-nav-right]')
        .css('visibility', "hidden").addClass('ls-hidden');
    ...
    registerArrows: function() {
      var self = this;
      jQuery((self.jQuerysliderWrap).find('[class^=ls-nav-]')).on('click', function() {
        self.setNextPanel(jQuery(this).attr('class').split(' ')[0].split('-')[2]);
      });
    },
    ...
    registerCrossLinks: function() {
      var self = this;
      // Find cross links
      self.crosslinks = jQuery('[data-liquidslider-ref*=' + (self.sliderId).split('#')[1] + ']');
      (self.crosslinks).on('click', function(e) {
        if (self.options.autoSlide === true)
          self.startAutoSlide(true);
        self.setNextPanel(self.getPanelNumber((jQuery(this).attr('href').split('#')[1]), self.options.panelTitleSelector));
        e.preventDefault();
      });
      self.updateClass();
    },
    ...
    registerTouch: function() {
      ...
      jQuery(self.sliderId + ' .panel').swipe(args);
    },
    ...
    registerKeyboard: function() {
      ...
      jQuery(document).keydown(function(event) {
        ...
        if (event.target.type !== 'textarea' && event.target.type !== 'textbox') {
          ...
          jQuery.each(self.options.panelKeys, function(index, value) {
            if (key === value) {
    ...
    adjustHeight: function(noAnimation, height, easing, duration) {
      ...
      if (noAnimation || self.useCSS) {
        ...
        (self.jQuerysliderId).height(height);
        if (noAnimation) self.configureCSSTransitions();
        return;
      }
      (self.jQuerysliderId).animate({
        'height': height + 'px'
      }, {
    ...
    getHeight: function(height) {
      var self = this;
      height = height || self.jQuerypanelClass.eq(self.sanatizeNumber(self.nextPanel) - 1).outerHeight(true);
    ...
    addPreloader: function() {
      var self = this;
      jQuery(self.sliderId + '-wrapper').append('<div class="ls-preloader"></div>');
    },

    removePreloader: function() {
      var self = this;
      jQuery(self.sliderId + '-wrapper .ls-preloader').fadeTo('slow', 0, function() {
        jQuery(this).remove();
      });
    },
    ...
    init: function(options, elem) {
      ...
      self.sliderId = '#' + (self.jQueryelem).attr('id');
      self.jQuerysliderId = jQuery(self.sliderId);

      // Set the options
      self.options = jQuery.extend({}, jQuery.fn.liquidSlider.options, options);
      ...
      if (!self.options.responsive && self.options.dynamicArrows)
        self.jQuerysliderWrap.width(self.jQuerysliderId.outerWidth(true) +
    ...
    build: function() {
      ...
      if ((self.jQuerysliderId).parent().attr('class') !== 'ls-wrapper') {
        (self.jQuerysliderId).wrap('<div id="' +
          (self.jQueryelem).attr('id') +
          '-wrapper" class="ls-wrapper"></div>');
      }
      // Cache the wrapper
      self.jQuerysliderWrap = jQuery(self.sliderId + '-wrapper');

      if (self.options.preloader) self.addPreloader();

      // Add the .panel class to the individual panels
      jQuery(self.sliderId).children().addClass((self.jQueryelem).attr('id') + '-panel panel');
      self.panelClass = self.sliderId + ' .' + (self.jQueryelem).attr('id') + '-panel:not(.clone)';
      self.jQuerypanelClass = jQuery(self.panelClass);

      // Wrap all panels in a div, and wrap inner content in a div (not backwards compatible)
      (self.jQuerypanelClass).wrapAll('<div class="panel-container"></div>');
      (self.jQuerypanelClass).wrapInner('<div class="panel-wrapper"></div>');
      self.panelContainer = (self.jQuerypanelClass).parent();
      self.jQuerypanelContainer = self.panelContainer;

      // If using fade transition, add the class here and disable other options.
      if (self.options.slideEaseFunction === "fade") {
        (self.jQuerypanelClass).addClass('fade');
        self.options.continuous = false;
        self.fade = true;
      }
      ...
      isAbsolute = ((self.jQueryleftArrow) && (self.jQueryleftArrow).css('position') === 'absolute') ? 0 : 1;

      // Set slider width
      self.totalSliderWidth = (self.jQuerysliderId).outerWidth(true) +
        (jQuery(self.jQueryleftArrow).outerWidth(true)) * isAbsolute +
        (jQuery(self.jQueryrightArrow).outerWidth(true)) * isAbsolute;
      jQuery((self.jQuerysliderWrap)).css('width', self.totalSliderWidth);
      ...
      if (self.options.continuous) {
        (self.jQuerypanelContainer).prepend((self.jQuerypanelContainer).children().last().clone().addClass('clone'));
        (self.jQuerypanelContainer).append((self.jQuerypanelContainer).children().eq(1).clone().addClass('clone'));
      }
      var clonedCount = (self.options.continuous) ? 2 : 0;

      // Count the number of panels and get the combined width
      self.panelCount = jQuery(self.panelClass).length;
      self.panelCountTotal = (self.fade) ? 1 : self.panelCount + clonedCount;
      self.panelWidth = jQuery(self.panelClass).outerWidth();
      self.totalWidth = self.panelCountTotal * self.panelWidth;

      // Apply the width to the panel container
      jQuery(self.sliderId + ' .panel-container').css('width', self.totalWidth);

      //How far should we slide?
      self.slideDistance = (self.options.responsive) ? 100 : jQuery(self.sliderId).outerWidth();
      if (self.useCSS) {
    ...
    configureCSSTransitions: function(slide, height) {
      ...
      if (self.useCSS) {
        ...
        jQuery(self.panelContainer).css({
    ...
    transitionFade: function() {
      var self = this;
      jQuery(self.panelClass).eq(self.nextPanel)
        .fadeTo(self.options.fadeInDuration, 1.0).css('z-index', 1);
      jQuery(self.panelClass).eq(self.prevPanel)
    ...
    events: function() {
      ...
      (self.jQuerysliderWrap).find('*').on('click', function() {
        if (self.options.forceAutoSlide)
          self.startAutoSlide(true);
    ...
    getPanelNumber: function(input, searchTerm) {
      ...
      (self.jQuerypanelClass).each(function(i) {
        title = self.convertRegex(jQuery(this).find(searchTerm).text());
        if (title === output) {
          output = i + 1;
        }
      });
    ...
    getFromPanel: function(searchTerm, panelNumber) {
      ...
      return self.convertRegex(self.jQuerypanelClass.find(searchTerm).eq(panelNumber).text());
    },
    ...
    updateClass: function() {
      var self = this;
      if (self.options.dynamicTabs) {
        jQuery((self.jQuerysliderWrap)).find('.tab' + self.sanatizeNumber(self.nextPanel) + ':first a')
          .addClass('current')
      ...
      self.jQuerypanelClass.eq(self.nextPanel)
    ...
    transitionOutAnimateCSS: function() {
      var self = this;
      jQuery(self.panelClass).removeClass(self.options.animateIn + ' animated');
      jQuery(self.panelClass).eq(self.prevPanel).addClass('animated ' + self.options.animateOut);
    ...
    transitionInAnimateCSS: function() {
      ...
      jQuery(self.panelClass).removeClass(self.options.animateOut + ' animated');
      jQuery(self.panelClass).eq(self.nextPanel).addClass('animated ' + self.options.animateIn);
    ...
    getHeighestPanel: function() {
      ...
      self.jQuerypanelClass.each(function() {
  ...
  jQuery.fn.liquidSlider = function(options) {
    return this.each(function() {
      var slider = Object.create(Slider);
      slider.init(options, this);
      jQuery.data(this, 'liquidSlider', slider);
    });
  };

  jQuery.fn.liquidSlider.options = {
    autoHeight: true,
    minHeight: 0,
